...
class AuditController
{
    ...
    private static function scope(array $u): array
    {
        ...
        return ["organization_id IN ($in)", array_map('intval', $ids)];
    }
    ...
    public static function index(): void
    {
        ...
        if ($org > 0) { self::requireOrgAccess($u, $org); $where[] = 'organization_id = ?'; $params[] = $org; }

        $q = trim((string) Http::query('q', ''));
        if ($q !== '') { $where[] = '(user_email LIKE ? OR entity LIKE ? OR entity_id LIKE ?)'; $like = '%' . $q . '%'; array_push($params, $like, $like, $like); }
        ...
        $total = (int) Database::scalar("SELECT COUNT(*) FROM audit_logs WHERE $w", $params);
        $rows = Database::all(
    ...
    public static function actions(): void
    {
        $u = Auth::requirePerm('audit.view');
        [$scope, $params] = self::scope($u);
        $rows = Database::all("SELECT DISTINCT action FROM audit_logs WHERE $scope ORDER BY action", $params);
