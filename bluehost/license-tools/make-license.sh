...
#   ./make-license.sh --domain acme.com --customer "ACME Inc" --edition standard --expires 2027-09-12
#
...
# and stops working after --expires (omit for a perpetual license).
set -euo pipefail
cd "$(dirname "$0")"
KEY="keys/vendor-private.pem"

DOMAIN=""; CUSTOMER=""; EDITION="standard"; EXPIRES=""
while [ $# -gt 0 ]; do
  case "$1" in
...
ISSUED="$(date +%Y-%m-%d)"
PAYLOAD="{\"customer\":\"$CUSTOMER\",\"domain\":\"$DOMAIN\",\"edition\":\"$EDITION\",\"issued\":\"$ISSUED\",\"expires\":$EXP_JSON}"
