...
export default function SelfServicePage() {
  ...
  const [assessments, setAssessments] = useState<Record<number, { score: string; comments: string; ratings?: Record<number, { score: string; comment: string }> }>>({});
  ...
  async function submitAssessment(id: number) {
    ...
    try { await apiFetch(`/me/performance-reviews/${id}/self-assessment`, { method: "POST", body: JSON.stringify({ self_score: Number(a.score), employee_comments: a.comments, ratings: Object.entries(a.ratings || {}).map(([itemId, x]) => ({ id: Number(itemId), score: Number(x.score), comment: x.comment })) }) }); setMsg("Self-assessment submitted to your supervisor."); load(); }
    catch (e: any) { setErr(e.message); }
  }
    ...
    <AppShell>
      ...
      {tab === "Performance" ? (
        <Section title={`My performance reviews (${reviews.length})`}><div className="space-y-4">
          {reviews.map((r) => { const editable = ["DRAFT", "SELF_SUBMITTED", "REJECTED"].includes(r.status); const a = assessments[r.id] || { score: String(r.self_score ?? ""), comments: r.employee_comments || "", ratings: Object.fromEntries((r.items||[]).map((x:any)=>[x.id,{score:String(x.self_score??""),comment:x.employee_comment||""}])) }; return <div key={r.id} className="rounded-lg border border-slate-200 p-4"><div className="flex flex-wrap justify-between gap-2"><div><div className="font-medium">{r.cycle_name}</div><div className="text-xs text-slate-400">{r.period_start || "—"} to {r.period_end || "—"} · Rating scale {r.rating_min}–{r.rating_max}</div></div><span className="badge bg-slate-100 text-slate-600">{r.status}</span></div><div className="mt-3 grid gap-3 sm:grid-cols-3"><div><div className="text-xs text-slate-400">Self score</div><div className="font-semibold">{r.self_score ?? "—"}</div></div><div><div className="text-xs text-slate-400">Supervisor score</div><div className="font-semibold">{r.supervisor_score ?? "—"}</div></div><div><div className="text-xs text-slate-400">Final rating</div><div className="font-semibold">{r.final_rating ?? "—"}</div></div></div><div className="mt-3 space-y-2">{(r.items||[]).map((it:any)=>{const x=a.ratings?.[it.id]||{score:String(it.self_score??""),comment:it.employee_comment||""};return <div key={it.id} className="rounded-lg bg-slate-50 p-3"><div className="flex justify-between gap-2"><div><div className="font-medium text-sm">{it.title}</div><div className="text-xs text-slate-400">{it.section_title} · {it.weight}% · {it.description}</div></div>{it.employee_rates?<input type="number" min={r.rating_min} max={r.rating_max} step="0.1" disabled={!editable} className="input w-24" value={x.score} onChange={(e)=>setAssessments({...assessments,[r.id]:{...a,ratings:{...(a.ratings||{}),[it.id]:{...x,score:e.target.value}}}})}/>:<span className="text-xs text-slate-400">Supervisor only</span>}</div>{it.employee_rates?<input disabled={!editable} className="input mt-2" placeholder="Comment for this criterion" value={x.comment} onChange={(e)=>setAssessments({...assessments,[r.id]:{...a,ratings:{...(a.ratings||{}),[it.id]:{...x,comment:e.target.value}}}})}/>:null}{it.supervisor_score!=null?<div className="mt-2 text-xs text-slate-600">Supervisor: {it.supervisor_score}{it.supervisor_comment?` — ${it.supervisor_comment}`:""}</div>:null}</div>})}</div>{r.supervisor_comments ? <p className="mt-2 text-sm"><strong>Supervisor:</strong> {r.supervisor_comments}</p> : null}{editable ? <div className="mt-3"><textarea className="input" placeholder="Overall achievements, goals, and comments" value={a.comments} onChange={(e) => setAssessments({ ...assessments, [r.id]: { ...a, comments: e.target.value } })} /><button className="btn-primary mt-2" onClick={() => submitAssessment(r.id)}>Submit self-assessment</button></div> : null}{r.status === "APPROVED" ? <button className="btn-primary mt-3" onClick={async () => { try { await apiFetch(`/me/performance-reviews/${r.id}/acknowledge`, { method: "POST" }); setMsg("Review acknowledged."); load(); } catch (e: any) { setErr(e.message); } }}>Acknowledge review</button> : null}</div>; })}
